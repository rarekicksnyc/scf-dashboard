...
export interface PolicySellerPremium {
  sellerId: string;
  generated: number; // insurer-rate premium this seller generated under the policy (FYTD)
  topUp: number; // this seller's share of the shortfall (pro-rata to generated)
}
...
export interface PolicyPremiumStatus {
  ...
  shortfall: number; // max(0, minimum − generated) — the year-end top-up
  sellers: PolicySellerPremium[]; // per-seller generated + attributed top-up (usage-weighted)
  fyStart: string;
}
...
export function policyPremiumStatus(asOf: string): PolicyPremiumStatus[] {
  ...
  const gen = new Map<string, number>(); // policyId → total premium generated
  const bySeller = new Map<string, Map<string, number>>(); // policyId → sellerId → generated
  for (const t of listBookedTransactions()) {
    ...
    for (const a of t.insurerAllocations) {
      const prem = a.amount * ((a.insurerRateBps ?? 0) / 10000) * tt;
      gen.set(a.policyId, (gen.get(a.policyId) ?? 0) + prem);
      let m = bySeller.get(a.policyId);
      if (!m) { m = new Map(); bySeller.set(a.policyId, m); }
      m.set(t.sellerId, (m.get(t.sellerId) ?? 0) + prem);
    }
  }
  return activePolicies()
    ...
    .map((p) => {
      ...
      const shortfall = Math.max(0, p.minimumPremium! - generatedFYTD);
      // Attribute the shortfall to the sellers who used the policy, pro-rata to the
      // premium each generated (their share of the policy's insured turnover). With
      // no usage the shortfall stays unattributed (empty list) until a seller books.
      const sellers: PolicySellerPremium[] = [...(bySeller.get(p.id) ?? new Map<string, number>()).entries()]
        .map(([sellerId, generated]) => ({
          sellerId,
          generated,
          topUp: shortfall > 0 && generatedFYTD > 0 ? shortfall * (generated / generatedFYTD) : 0,
        }))
        .sort((a, b) => b.generated - a.generated);
      return { policyId: p.id, insurerName: p.insurerName, policyNumber: p.policyNumber, minimumPremium: p.minimumPremium!, generatedFYTD, shortfall, sellers, fyStart };
    });
}
