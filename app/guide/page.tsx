...
const FLOW: { step: string; detail: string }[] = [
  ...
  { step: "Document & execute", detail: "Generate the purchase/commitment request and Schedule A, email the client, and upload the executed document. The signer is checked against the authorized signatory list. Once executed and signature-verified, loan ops (Operations) is notified in-app that the documents are ready to book, and can open the executed document and draft the loan-ops email." },
...
const SCREENS: [string, string][] = [
  ...
  ["Reports", "Exportable transaction and exposure reports — Portfolio Manager and Administrator roles only."],
  ["Creator mode", "Governed, no-code platform extension (Portfolio Manager and Administrator): add custom fields and reference registers, build KPI tiles from a safe formula, set advisory watch rules, add custom columns to the Transaction and Exposure reports (formula/text/dropdown), hide built-in report columns, and add file-template fields exposed as {{cf_key}} tokens. Every change is validated, audited, and reversible; nothing here changes the ledger or a control."],
...
const GLOSSARY: [string, string][] = [
  ...
  ["Skim", "Extra income the bank keeps on an investor participation, tracked as revenue and never shown to the investor. It has two parts, reported separately on Revenue: the funding basis (the bank funds the client at COF but pays the investor only the interpolated SOFR, keeping COF − SOFR) and the margin skim (the negotiated slice of margin, in bps, the bank retains rather than passing to the investor)."],
  ["Investor offer", "The package emailed to an investor for a participation: an investor Purchase Request (the client Purchase Request with the base rate and client margin replaced by the investor's own terms — interpolated SOFR + investor margin) and the investor Schedule A. Drafted from Transaction Flow; the skim and client margin never appear on it."],
  ["Insurer skim", "On an insured deal the client is charged an insurance rate (e.g. 90bps) but the insurer is paid a lower rate (e.g. 75bps); MUFG keeps the spread over the tenor as income. Entered per deal on the insurance allocation (client rate + insurer rate)."],
